...
// Save state (plain copy - ARM's SHA256H/SHA256H2 instructions consume
// state directly in {A,B,C,D}/{E,F,G,H} order. Unlike x86's SHA
// extensions (which pack state as {D,C,B,A}/{H,G,F,E} and need a
// PSHUFD-style reorder), ARM needs no shuffle here or at combine time
// below - the vtrn1q_u64/vtrn2q_u64 dance that used to be here was
// copied from an x86-style pattern that doesn't apply on ARM.
ABEF_SAVE = STATE0;
CDGH_SAVE = STATE1;
...
// Combine state: STATE0/STATE1 already hold the correct values after the
// 64 rounds above - just add back the pre-round state saved earlier.
// (The previous version overwrote STATE0/STATE1 with a reshuffle of the
// *saved* pre-round values before adding, which discarded the result of
// all 64 rounds - this was the fatal bug: every digest came out wrong
// because the compression's actual output was thrown away here.)
