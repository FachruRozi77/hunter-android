//============================================================================
// BTC PUZZLE HUNTER v4.1 - ULTIMATE ARM64 EDITION (FIXED)
...
//
// FIXES in v4.1:
// 1. SHA-256 padding: proper 64-bit big-endian length encoding
// 2. Removed system("clear") - uses ANSI cursor positioning
// 3. Fast random generation for small ranges
// 4. Added debug logging to file
// 5. Added ModInv zero-check safety
// 6. Added fflush after config output
//============================================================================
...
// FIX v4.1: Proper SHA-256 padding for single 33-byte block
// CRITICAL FIX: 64-bit bit length must be in big-endian in the LAST 8 bytes
static inline void sha256_arm64_single(const uint8_t* data, uint8_t out[32]) {
    ...
    block[33] = 0x80;
    memset(block + 34, 0, 64 - 34 - 8);  // Zero until last 8 bytes

    // FIX: 64-bit bit length in BIG-ENDIAN (last 8 bytes of block)
    // 33 bytes * 8 = 264 bits
    uint64_t bit_len = 33ULL * 8;
    for (int i = 0; i < 8; i++) {
        block[56 + i] = (uint8_t)((bit_len >> (56 - i * 8)) & 0xff);
    }
...
// FIX v4.1: Fast random generation for small ranges
// If range fits in 32 bits, use direct 32-bit random modulo
    ...
    void run() {
        ...
        std::thread statThread([this, tStart, &threadStats]() {
            ...
            while (!g_matchFound.load(std::memory_order_relaxed) && !g_shutdownRequested.load()) {
                ...
                // FIX v4.1: Use ANSI escape instead of system("clear")
                // Move cursor to top-left and clear from cursor to end
                std::cout << "\033[2J\033[H";

                std::cout << "+==============================================================+\n"
                          << "| BTC PUZZLE HUNTER v4.1 - ULTIMATE ARM64 NEON+SHA (FIXED)   |\n"
                          << "+==============================================================+\n"
...
int main(int argc, char* argv[]) {
    initDebugLog();
    debugLog("=== BTC Puzzle Hunter v4.1 Starting ===");
    ...
    std::cout.flush();  // FIX v4.1: Ensure config output is visible
    ...
    std::cout.flush();  // FIX v4.1: Ensure message is visible
