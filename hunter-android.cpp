// ============================================================================
// BTC PUZZLE HUNTER v3.0 - ULTIMATE ARM64 EDITION
// Combines: Jean Luc Pons SECP256K1 (batch inversion) + libsecp256k1 robustness
...
// ============================================================================
// SIGNAL HANDLING (From user's C code - improved)
// ============================================================================
...
// ============================================================================
// THREAD AFFINITY (From user's C code - C++ wrapper)
// ============================================================================
...
// ============================================================================
// BATCH HASH160 (EVP context reuse)
// ============================================================================
...
static inline void generateRandomIntInRange(const Int& start, const Int& rangeSize, Int& result, FastRandom& rng) {
    ...
    Int rangeCopy; rangeCopy.Set((Int*)&rangeSize); result.Mod(&rangeCopy);
    Int startCopy; startCopy.Set((Int*)&start); result.Add(&startCopy);
}

// ============================================================================
// MEMORY MONITORING (From user's C code)
// ============================================================================
...
public:
    ...
    void setRange(const std::string& startHex, const std::string& endHex) {
        startRange = hexToInt(startHex); endRange = hexToInt(endHex); rangeSize.Sub(&endRange, &startRange);
    }
    void initialize() {
    ...
    void workerThread(int tid, ThreadStats& stats) {
        // Set thread affinity (from user's C code)
    ...
    void run() {
        ...
        std::thread statThread([this, tStart, &threadStats]() {
            while (!g_matchFound.load(std::memory_order_relaxed) && !g_shutdownRequested.load()) {
                ...
                system("clear");
                std::cout << "╔══════════════════════════════════════════════════════════════╗\n"
                          << "║         BTC PUZZLE HUNTER v3.0 - ULTIMATE ARM64               ║\n"
                          << "╠══════════════════════════════════════════════════════════════╣\n"
...
int main(int argc, char* argv[]) {
    // Signal handlers (from user's C code)
    ...
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n"
              << "║     BTC Puzzle Hunter v3.0 - ULTIMATE ARM64                    ║\n"
              << "║     Batch Inversion + Thread Affinity + Thermal Mgmt           ║\n"
              << "╚══════════════════════════════════════════════════════════════╝\n\n";
